...
import {
  Send, Play, ChevronRight, Loader2, Bot, User, StopCircle,
  History, Bookmark, BookmarkCheck, BarChart3,
} from 'lucide-react'
...
export default function Chat() {
  ...
  const [analysisMetrics, setAnalysisMetrics] = useState<{ name: string; value: string }[]>([])
  const [conversations, setConversations] = useState<any[]>([])
  const [bookmarks, setBookmarks] = useState<any[]>([])
  const [showPanel, setShowPanel] = useState<'none' | 'analysis' | 'history' | 'bookmarks'>('none')
  ...
  function runAnalysis() {
    ...
    createAnalysisStream(
      id, 'make verify', token,
      (content) => setAnalysisOutput((prev) => [...prev, content]),
      (content) => setAnalysisOutput((prev) => [...prev, `[错误] ${content}`]),
      (_exitCode, results) => {
        setIsAnalyzing(false)
        if (results?.metrics?.length > 0) {
          setAnalysisMetrics(results.metrics)
          setAnalysisOutput((prev) => [
            ...prev,
            `\n分析完成，检测到 ${results.metrics.length} 个指标`,
          ])
        } else {
  ...
  async function addBookmark() {
    ...
    try {
      ...
      await fetch('/api/chat/bookmarks', {
        ...
        body: JSON.stringify({ conversationId, projectId: id, label: bookmarkLabel || '书签' }),
      })
  ...
  async function removeBookmark(bookmarkId: string) {
    try {
      const token = localStorage.getItem('token')
      await fetch(`/api/chat/bookmarks/${bookmarkId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      })
      loadBookmarks()
    ...
    <div className="flex h-[calc(100vh-8rem)] gap-6">
      ...
      <div className="flex flex-1 flex-col rounded-xl border border-[#334155] bg-[#1e293b] overflow-hidden">
          ...
          <div className="flex items-center gap-2">
                ...
                {showBookmarkInput ? (
                  <div className="flex items-center gap-1">
                    <input
                      ...
                      placeholder="书签名称..."
                      className="w-28 rounded-lg border border-[#334155] bg-[#0f172a] px-2 py-1 text-xs text-[#f1f5f9] placeholder-[#64748b] outline-none"
                      onKeyDown={(e) => e.key === 'Enter' && addBookmark()}
                    />
                    <button onClick={addBookmark} className="text-[#22c55e] text-xs">保存</button>
                    <button onClick={() => setShowBookmarkInput(false)} className="text-[#94a3b8] text-xs">取消</button>
                  </div>
                ) : (
                  <button
            ...
            <button
              ...
              className={`rounded-lg border p-2 transition-colors ${
                showPanel === 'history' ? 'border-[#3b82f6] text-[#3b82f6]' : 'border-[#334155] text-[#94a3b8] hover:bg-[#334155]'
              }`}
              title="对话历史"
            >
            ...
            <button
              onClick={() => setShowPanel(showPanel === 'bookmarks' ? 'none' : 'bookmarks')}
              className={`rounded-lg border p-2 transition-colors ${
                showPanel === 'bookmarks' ? 'border-[#3b82f6] text-[#3b82f6]' : 'border-[#334155] text-[#94a3b8] hover:bg-[#334155]'
              }`}
              title="书签"
            >
        ...
        <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`message-enter flex gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
              ...
              <div className={`max-w-[80%] rounded-xl px-4 py-3 ${
                msg.role === 'user' ? 'bg-[#3b82f6] text-white' : 'bg-[#0f172a] text-[#f1f5f9]'
              }`}>
          ...
          {analysisMetrics.length > 0 && !isAnalyzing && (
            <div className="message-enter flex gap-3">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#3b82f6]/10">
                <BarChart3 className="h-4 w-4 text-[#3b82f6]" />
              </div>
              <div className="max-w-[80%]">
                <ResultsChart metrics={analysisMetrics} />
              </div>
            </div>
          )}
      ...
      {/* Right Panel */}
      {showPanel !== 'none' && (
        <div className="w-80 rounded-xl border border-[#334155] bg-[#1e293b] overflow-hidden shrink-0">
          <div className="flex items-center justify-between border-b border-[#334155] px-4 py-3">
            <h3 className="text-sm font-medium text-[#f1f5f9]">
              {showPanel === 'analysis' ? '分析输出' : showPanel === 'history' ? '对话历史' : '书签'}
            </h3>
            <button
              onClick={() => setShowPanel('none')}
              className="text-[#64748b] hover:text-[#f1f5f9] text-lg leading-none"
            >
              ×
            </button>
          </div>

          <div className="overflow-y-auto p-4 space-y-3 max-h-[calc(100vh-16rem)]">
            {showPanel === 'analysis' && (
              <>
                  ...
                  conversations.map((conv) => (
                    <button
                      ...
                      className={`w-full rounded-lg bg-[#0f172a] p-3 text-left hover:bg-[#334155] transition-colors ${
                        conv.id === conversationId ? 'ring-1 ring-[#3b82f6]' : ''
                      }`}
                    >
            ...
            {showPanel === 'bookmarks' && (
              <>
                {bookmarks.length === 0 ? (
                  <p className="text-sm text-[#64748b]">暂无书签</p>
                ) : (
                  bookmarks.map((bm) => (
                    <div key={bm.id} className="rounded-lg bg-[#0f172a] p-3">
                      <div className="flex items-center justify-between">
                        <p className="text-sm text-[#f1f5f9] truncate">{bm.label || '书签'}</p>
                        <button
                      ...
                      {bm.conversation_title && (
                        <p className="text-xs text-[#64748b] mt-1 truncate">{bm.conversation_title}</p>
                      )}
